...
QsciSciCallTip::QsciSciCallTip(QWidget *parent, QsciScintillaQt *sci_)
    : QWidget(parent, Qt::WindowFlags(Qt::Popup | Qt::FramelessWindowHint |
...
void QsciSciCallTip::paintEvent(QPaintEvent *) {
  Scintilla::Surface *surfaceWindow =
      Scintilla::Surface::Allocate(SC_TECHNOLOGY_DEFAULT);
...
void QsciSciPopup::on_triggered(int cmd) { sci->Command(cmd); }
...
int QsciSciListBox::find(const QString &prefix) {
  QList<QListWidgetItem *> itms =
      findItems(prefix, Qt::MatchStartsWith | Qt::MatchCaseSensitive);

  if (itms.size() == 0) return -1;
...
void QsciSciListBox::mouseReleaseEvent(QMouseEvent *) { lbx->handleRelease(); }
