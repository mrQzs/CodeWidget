...
QStringList QsciAPIsPrepared::apiWords(int api_idx, const QStringList &wseps,
  ...
  if (strip_image) {
    ...
    if (tail >= 0) base.truncate(tail);
  }

  if (wseps.isEmpty()) return QStringList(base);
...
const QEvent::Type WorkerStarted =
    static_cast<QEvent::Type>(QEvent::User + 1012);

// The user event type that signals that the worker thread has finished.
const QEvent::Type WorkerFinished =
    static_cast<QEvent::Type>(QEvent::User + 1013);

// The user event type that signals that the worker thread has aborted.
const QEvent::Type WorkerAborted =
    static_cast<QEvent::Type>(QEvent::User + 1014);
...
void QsciAPIsWorker::run() {
  ...
  for (int a = 0; a < prepared->raw_apis.count(); ++a) {
    ...
    for (int w = 0; w < words.count(); ++w) {
      ...
      if (!cs && wil.count() == 0) prepared->cdict[word.toUpper()] = word;
  ...
  QApplication::postEvent(proxy,
                          new QEvent(abort ? WorkerAborted : WorkerFinished));
}
...
void QsciAPIs::clear() { apis.clear(); }
  ...
  if (origin_len > 0) {
    ...
    if (start_new == end_new) {
      ...
      while (start_new < end_new) {
        ...
        if (origin == prep->raw_apis.end()) break;
    ...
    if (origin == prep->raw_apis.end()) origin_len = 0;
  }

  if (origin_len == 0) path.truncate(0);
...
bool QsciAPIs::originStartsWith(const QString &path, const QString &wsep) {
  ...
  if (!orig.startsWith(path)) return false;
...
void QsciAPIs::updateAutoCompletionList(const QStringList &context,
  ...
  if (origin_len > 0) {
    ...
    if (new_context.last().isEmpty())
      lastCompleteWord(new_context[new_context.count() - 2], with_context,
                       unambig);
    else
...
const QsciAPIs::WordIndexList *QsciAPIs::wordIndexOf(
...
void QsciAPIs::lastCompleteWord(const QString &word, QStringList &with_context,
                                bool &unambig) {
  // Get the possible API entries if any.
  const WordIndexList *wl = wordIndexOf(word);

  if (wl) addAPIEntries(*wl, true, with_context, unambig);
}

// Add auto-completion words based on the last partial word entered.
void QsciAPIs::lastPartialWord(const QString &word, QStringList &with_context,
                               bool &unambig) {
  if (lexer()->caseSensitive()) {
    QMap<QString, WordIndexList>::const_iterator it =
        prep->wdict.lowerBound(word);

    while (it != prep->wdict.end()) {
      if (!it.key().startsWith(word)) break;
...
void QsciAPIs::autoCompletionSelected(const QString &selection) {
  ...
  origin =
      std::lower_bound(prep->raw_apis.begin(), prep->raw_apis.end(), owords);
...
void QsciAPIs::addAPIEntries(const WordIndexList &wl, bool complete,
  ...
  for (int w = 0; w < wl.count(); ++w) {
    ...
    if (complete) {
      ...
      if (++idx >= api_words.count()) continue;
    }
    ...
    if (!with_context.contains(api_word)) with_context.append(api_word);
  }
}
...
QStringList QsciAPIs::callTips(const QStringList &context, int commas,
  ...
  if (origin_len > 0) {
    ...
    if (ctstart) ctstart += wsep.length();
    ...
    if (wil)
      for (int i = 0; i < wil->count(); ++i) {
        ...
        // context.
        if (wi.second != awords.count() - 1) continue;
        ...
        if (!enoughCommas(api, commas)) continue;
...
void QsciAPIs::cancelPreparation() { deleteWorker(); }
...
bool QsciAPIs::loadPrepared(const QString &filename) {
  ...
  if (cpdata.count() == 0) return false;
  ...
  if (vers > PreparedDataFormatVersion) return false;
...
QString QsciAPIs::defaultPreparedName() const { return prepName(QString()); }
...
QString QsciAPIs::prepName(const QString &filename, bool mkpath) const {
  ...
  if (qsci)
    ...
    if (mkpath && !pd.exists(qsci_dir) && !pd.mkdir(qsci_dir)) return QString();
...
QStringList QsciAPIs::installedAPIFiles() const {
  ...
  QFileInfoList flist =
      apidir.entryInfoList(filters, QDir::Files, QDir::IgnoreCase);

  foreach (QFileInfo fi, flist) filenames << fi.absoluteFilePath();
