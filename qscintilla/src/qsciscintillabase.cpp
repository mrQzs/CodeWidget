...
QsciScintillaBase *QsciScintillaBase::pool() { return poolList.first(); }

// Tell Scintilla to update the scroll bars.  Scintilla should be doing this
// itself.
void QsciScintillaBase::setScrollBars() { sci->SetScrollBars(); }
...
long QsciScintillaBase::SendScintilla(unsigned int msg, long wParam) const {
  return sci->WndProc(msg, static_cast<uptr_t>(wParam), static_cast<sptr_t>(0));
}
...
void *QsciScintillaBase::SendScintillaPtrResult(unsigned int msg) const {
  return reinterpret_cast<void *>(
      sci->WndProc(msg, static_cast<uptr_t>(0), static_cast<sptr_t>(0)));
}
...
bool QsciScintillaBase::focusNextPrevChild(bool next) {
  if (!sci->pdoc->IsReadOnly()) return false;
...
void QsciScintillaBase::keyPressEvent(QKeyEvent *e) {
  ...
  if (e->modifiers() & Qt::ShiftModifier) modifiers |= SCMOD_SHIFT;

  if (e->modifiers() & Qt::ControlModifier) modifiers |= SCMOD_CTRL;

  if (e->modifiers() & Qt::AltModifier) modifiers |= SCMOD_ALT;

  if (e->modifiers() & Qt::MetaModifier) modifiers |= SCMOD_META;
...
QsciScintillaBase::ScintillaBytes QsciScintillaBase::textAsBytes(
    const QString &text) const {
  if (sci->IsUnicodeMode()) return text.toUtf8();
...
QString QsciScintillaBase::bytesAsText(const char *bytes) const {
  if (sci->IsUnicodeMode()) return QString::fromUtf8(bytes);
...
void QsciScintillaBase::mousePressEvent(QMouseEvent *e) {
  ...
  if (e->button() == Qt::LeftButton || e->button() == Qt::RightButton) {
    ...
    if (e->button() == Qt::LeftButton)
      sci->ButtonDownWithModifiers(
          pt, clickTime, QsciScintillaQt::ModifierFlags(shift, ctrl, alt));
    else
      sci->RightButtonDownWithModifiers(
          pt, clickTime, QsciScintillaQt::ModifierFlags(shift, ctrl, alt));
...
void QsciScintillaBase::mouseReleaseEvent(QMouseEvent *e) {
  ...
  if (sci->HaveMouseCapture()) {
    ...
    sci->ButtonUpWithModifiers(
        pt, 0, QsciScintillaQt::ModifierFlags(false, ctrl, false));
  }

  if (!sci->pdoc->IsReadOnly() && !sci->PointInSelMargin(pt) &&
      qApp->autoSipEnabled()) {
    QStyle::RequestSoftwareInputPanel rsip = QStyle::RequestSoftwareInputPanel(
...
// Handle paint events.
void QsciScintillaBase::paintEvent(QPaintEvent *e) { sci->paintEvent(e); }

// Handle resize events.
void QsciScintillaBase::resizeEvent(QResizeEvent *) { sci->ChangeSize(); }
...
void QsciScintillaBase::handleVSb(int value) { sci->ScrollTo(value); }

// Handle the horizontal scrollbar.
void QsciScintillaBase::handleHSb(int value) { sci->HorizontalScrollTo(value); }
...
void QsciScintillaBase::dropEvent(QDropEvent *e) {
  if (e->mimeData()->hasUrls()) {
    ...
    foreach (const QUrl &url, e->mimeData()->urls()) emit SCN_URIDROPPED(url);
  ...
  std::string dest =
      Scintilla::Document::TransformLineEnds(s, len, sci->pdoc->eolMode);

  sci->DropAt(sci->posDrop, dest.c_str(), dest.length(), moving, rectangular);
...
QByteArray QsciScintillaBase::fromMimeData(const QMimeData *source,
...
QMimeData *QsciScintillaBase::toMimeData(const QByteArray &text,
                                         bool rectangular) const {
...
void QsciScintillaBase::connectVerticalScrollBar() {
  connect(verticalScrollBar(), SIGNAL(valueChanged(int)), SLOT(handleVSb(int)));
}
